...
export default function SearchReceiverPage(props) {
  ...
  const pushReceiverData = (pushUserId) => {
    console.log(`Test. Coba lihat ${pushUserId}`);
    router.push(`userprofile/transaction/${pushUserId}`);
  };
          ...
          <div className={`row justify-content-between`}>
            ...
            <div className={`col-lg-9 col-md-6 col-sm-9 col-xs-12 `}>
              ...
              {userReceiverData.map((item, index) => {
                ...
                const pushUserId = item.user_id;
                return (
                  <button
                    className={`${styles.whiteBackgroundWithBorderRadius} ${styles.buttonReceiverData} mt-4 w-100`}
                    key={index}
                    onClick={() => pushReceiverData(pushUserId)}
                  >
                    <div className="p-4">
                      <span className="d-block fw-bold pb-1">
                        {item.user_name}
                      </span>
                      <span className="d-block fw-bold pt-1">
                        {item.user_phone}
                      </span>
                    </div>
                  </button>
